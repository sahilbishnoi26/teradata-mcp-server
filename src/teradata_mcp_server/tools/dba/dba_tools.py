...
def handle_get_dba_tableSqlList(conn: TeradataConnection, table_name: str, no_days: Optional[int],  *args, **kwargs):
    logger.debug(f"Tool: handle_get_dba_tableSqlList: Args: table_name: {table_name}")
    ...
    with conn.cursor() as cur:
        ...
        metadata = {
            "tool_name": "get_dba_tableSqlList",
...
#       user_name (str) - name of the user 
#     Returns: formatted response with list of QueryText and UserIDs or error message    
def handle_get_dba_userSqlList(conn: TeradataConnection, user_name: Optional[str] | None, no_days: Optional[int],  *args, **kwargs):
    logger.debug(f"Tool: handle_get_dba_userSqlList: Args: user_name: {user_name}")
...
def handle_get_dba_tableSpace(conn: TeradataConnection, db_name: Optional[str] | None , table_name: Optional[str] | None, *args, **kwargs):
    logger.debug(f"Tool: handle_get_dba_tableSpace: Args: db_name: {db_name}, table_name: {table_name}")
...
def handle_get_dba_databaseSpace(conn: TeradataConnection, db_name: Optional[str] | None, *args, **kwargs):
    logger.debug(f"Tool: handle_get_dba_databaseSpace: Args: db_name: {db_name}")
    ...
    with conn.cursor() as cur:
        ...
        data = rows_to_json(cur.description, rows.fetchall())
        metadata = {
            "tool_name": "get_dba_databaseSpace",
            "db_name": db_name,
...
def handle_get_dba_databaseVersion(conn: TeradataConnection, *args, **kwargs):
    logger.debug("Tool: handle_get_dba_databaseVersion: Args: ")
    ...
    with conn.cursor() as cur:
        ...
        data = rows_to_json(cur.description, rows.fetchall())
        metadata = {
            "tool_name": "get_dba_databaseVersion",
            "total_rows": len(data)
        }
        return create_response(data, metadata)
...
def handle_get_dba_resusageSummary(conn: TeradataConnection, 
    ...
    logger.debug(f"Tool: handle_get_dba_resusageSummary: Args: dimensions: {dimensions}")
    ...
    with conn.cursor() as cur:   
        ...
        data = rows_to_json(cur.description, rows.fetchall())
        metadata = {
            "tool_name": "get_dba_resusageSummary",
            "total_rows": len(data) ,
            "comment": comment
        }
        return create_response(data, metadata)
...
def handle_get_dba_flowControl(conn: TeradataConnection, *args, **kwargs):
    logger.debug("Tool: handle_get_dba_flowControl: Args: ")
...
#       db_name (str) - name of the database
#       user_name (str) - name of the user
def handle_get_dba_tableUsageImpact(conn: TeradataConnection, db_name: Optional[str] = None, user_name: Optional[str] = None, *args, **kwargs):
    """
    Measure the usage of a table and views by users, this is helpful to understand what user and tables are driving most resource usage at any point in time.
    """
    logger.debug("Tool: handle_get_dba_tableUsageImpact: Args: ")
    if db_name:
    ...
    metadata = {
        "tool_name": "handle_get_dba_tableUsageImpact",
        "database": db_name,
...
def handle_get_dba_featureUsage(conn: TeradataConnection, *args, **kwargs):
    logger.debug("Tool: handle_get_dba_featureUsage: Args: ")
    ...
    with conn.cursor() as cur:
        ...
        data = rows_to_json(cur.description, rows.fetchall())
        metadata = {
            "tool_name": "get_dba_featureUsage",
            "total_rows": len(data) 
        }
        return create_response(data, metadata)    
...
def handle_get_dba_userDelay(conn: TeradataConnection, *args, **kwargs):
    logger.debug("Tool: handle_get_dba_userDelay: Args: ")
