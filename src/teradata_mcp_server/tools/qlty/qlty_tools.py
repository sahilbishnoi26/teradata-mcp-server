...
def handle_get_qlty_missingValues(conn: TeradataConnection, table_name: str, *args, **kwargs):
    logger.debug(f"Tool: handle_get_qlty_missingValues: Args: table_name: {table_name}")
    ...
    with conn.cursor() as cur:
        ...
        metadata = {
            "tool_name": "get_qlty_missingValues",
...
def handle_get_qlty_negativeValues(conn: TeradataConnection, table_name: str, *args, **kwargs):
    logger.debug(f"Tool: handle_get_qlty_negativeValues: Args: table_name: {table_name}")
    ...
    with conn.cursor() as cur:
        ...
        data = rows_to_json(cur.description, rows.fetchall())
        metadata = {
            "tool_name": "get_qlty_negativeValues",
            "table_name": table_name,
        }
        return create_response(data, metadata)
...
def handle_get_qlty_distinctCategories(conn: TeradataConnection, table_name: str, col_name: str, *args, **kwargs):
    logger.debug(f"Tool: handle_get_qlty_distinctCategories: Args: table_name: {table_name}, col_name: {col_name}")

    with conn.cursor() as cur: 
        rows = cur.execute(f"select * from TD_CategoricalSummary ( on {table_name} as InputTable using TargetColumns ('{col_name}')) as dt")
        data = rows_to_json(cur.description, rows.fetchall())
        metadata = {
            "tool_name": "get_qlty_distinctCategories",
            "table_name": table_name,
            "col_name": col_name,
...
def handle_get_qlty_standardDeviation(conn: TeradataConnection, table_name: str, col_name: str, *args, **kwargs):
    logger.debug(f"Tool: handle_get_qlty_standardDeviation: Args: table_name: {table_name}, col_name: {col_name}")
    ...
    with conn.cursor() as cur:
        ...
        data = rows_to_json(cur.description, rows.fetchall())
        metadata = {
            "tool_name": "get_qlty_standardDeviation",
            "table_name": table_name,
            "col_name": col_name,
